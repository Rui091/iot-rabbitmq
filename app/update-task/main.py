...
QUEUE_NAME = os.environ.get("QUEUE_NAME", "update_queue")
...
def callback(ch, method, properties, body):
    try:
        ...
        if message.get("action") == "update" and "task_id" in message and "status" in message:
            conn = get_db_connection()
            cur = conn.cursor()
            cur.execute(
                "UPDATE tasks SET status = %s WHERE task_id = %s",
                (message["status"], message["task_id"])
            )
            conn.commit()
            cur.close()
            conn.close()
            print(f"Successfully updated task {message['task_id']}")
